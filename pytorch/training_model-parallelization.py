...
if __name__ == "__main__":
    ...
    test_debug_folder_path = test_results_folder_path +'debug/'
    ...
    if test_debug_folder_path and not os.path.exists(test_debug_folder_path):
        os.makedirs(test_debug_folder_path)
    ...
    def get_batch(batch):
        ...
        if torch.cuda.is_available():
            ...
            batch_images = torch.Tensor(np.concatenate(batch_images)).cuda()

            batch_labels = torch.Tensor(np.concatenate(batch_labels)).cuda()

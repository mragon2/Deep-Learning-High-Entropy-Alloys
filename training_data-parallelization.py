...
if __name__ == "__main__":
    ...
    def distributed_train_step(global_batch):
        ...
        per_replica_losses = strategy.run(train_step, args=(global_batch,))
    ...
    def distributed_test_step(global_batch):
        ...
        per_replica_losses = strategy.run(test_step, args=(global_batch,))
    ...
    for epoch in range(first_epoch, first_epoch + num_epochs):
        ...
        for train_batch_index,train_batch in enumerate(train_dataset):
            ...
            global_train_batch = get_global_batch(train_batch)

            num_images = global_train_batch[0].shape[0]

            before = time.time()

            total_train_loss += distributed_train_step(global_train_batch)
        ...
        for test_batch_index,test_batch in enumerate(test_dataset):
            ...
            global_test_batch = get_global_batch(test_batch)

            total_test_loss += distributed_test_step(global_test_batch)

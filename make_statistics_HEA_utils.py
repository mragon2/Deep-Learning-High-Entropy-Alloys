...
class CHs_Distribution(object):
	...
	def get_CHs(self,lbl_single_element):

		peaks_pos = self.get_peaks_pos(lbl_single_element)
	...
	def get_CHs_distribution(self):
		...
		for e in range(self.n_elements):
			...
			for data_index,data_file in enumerate(os.listdir(self.path)):
				...
				CHs = self.get_CHs(lbl_single_element)
				...
				for ch in range(self.ch_min, self.ch_max + 1):
					...
					absolute_ch_distr_single_element[ch - 1, data_index] = n_ch
					relative_ch_distr_single_element[ch - 1, data_index] = np.round(n_ch/n_columns, decimals = 3)
		...
		for data_index, data_file in enumerate(os.listdir(self.path)):
			...
			for peaks_index,peaks_pos in enumerate(peaks_pos_all_elements):
				...
				for ce in range(self.n_elements):
					...
					I_absolute_ch_correlation_single_data[ce, peaks_index, 0] = ch + 1
					I_absolute_ch_correlation_single_data[ce,peaks_index,1] = I

					I_relative_ch_correlation_single_data[ce, peaks_index, 0] = (ch + 1)/ch_tot
